...
from custom_types import (RAGChunkAndSRC, RAGUpserResult, RAGSearchResult, RaGQueryResult)
...
async def rag_ingest_pdf(ctx: inngest.Context):
    ...
    def _upsert(chunks_and_src: RAGChunkAndSRC) -> RAGUpserResult:
        ...
        return RAGUpserResult(ingested=len(chunks))

    chunks_and_src = await ctx.step.run("load-and-chunk", lambda: _load(ctx), output_type=RAGChunkAndSRC)
    ingested = await ctx.step.run("embed-and-upsert", lambda: _upsert(chunks_and_src), output_type=RAGUpserResult)
...
async def rag_query_pdf_ai(ctx: inngest.Context):
    ...
    async def _generate_answer(found: RAGSearchResult, question: str) -> str:
